...
- 新增 0.8.4 Rust-kernel release-gate matrix，覆盖已选择的声纹计分、结果
  后处理与 artifact manifest helper 路径，并明确 hard-fail、rollback、CI
  和内部性能基线证据。
...
- 修复多 GPU / per-model lazy-load 选卡 (#20)：Docker Compose 默认请求所有可用
  NVIDIA GPU，不再通过 `CUDA_VISIBLE_DEVICES=0` 和 `count: 1` 把容器限制到单卡，
  也不会默认注入空的 `CUDA_VISIBLE_DEVICES`；如需限制可见卡，需通过 compose
  override 或显式 env 注入。
...
- internal live validation 覆盖 0.7.5 的空闲卸载、多 GPU lazy-load 选卡、固定
  `DEVICE=cuda:N` 不漂移、faster-whisper `device_index`、pyannote 本地
  snapshot/config 加载和默认 `MODEL_IDLE_TIMEOUT_SEC=180` 行为。
